...
interface FriendshipRepository : JpaRepository<Friendship, Long> {
    ...
    @Query(
        value = "SELECT F FROM Friendship AS F WHERE (F.userAsk.id = ?1 AND F.userAnswer.id = ?2) OR ( F.userAsk.id = ?2 AND F.userAnswer.id = ?1 ) "
    )
    fun findFriendshipByUsersIds(userAsk_id: Long, userAnswer_id: Long) : Optional<FriendshipProjection>
     ...
     */
    @Query(value = "SELECT COUNT(DISTINCT F.id) FROM Friendship AS F JOIN DateCity AS DC on F.userAnswer.id = DC.user.id JOIN DateCity AS DC2 on F.userAsk.id = DC2.user.id " +
            "WHERE F.answer= com.esei.grvidal.nightTimeApi.utils.AnswerOptions.YES AND ( DC.nextCity.id = ?2 AND DC.nextDate = ?3 AND F.userAsk.id = ?1 ) OR ( DC2.nextCity.id = ?2 AND DC2.nextDate = ?3 AND F.userAnswer.id = ?1 )"
    )
    fun getCountFriendsFromUserOnDate(idUser: Long, idCity: Long, date: LocalDate) : Int
